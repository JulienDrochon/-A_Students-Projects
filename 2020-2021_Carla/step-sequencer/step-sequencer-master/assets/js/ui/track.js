...
(function (App, $, setTimeout) {
  ...
  App.ui.Track = function (trackId, options) {
    ...
    function correctStepsCollection() {
      var stepsNeeded = stepsPerBeat * beatsPerMeasure * measures,
        i,
        iMax;
      ...
      if (stepsNeeded >= stepsCollection.length) {
        ...
        for (i = 0; i < iMax; i += 1) {
          stepsCollection.push($("<div>", { class: opts.stepClass })[0]);
        }
      } else {
    ...
    function redraw() {
      ...
      for (iMeasures = 0; iMeasures < measures; iMeasures += 1) {
        ...
        // $curMeasure[iMeasures].classList.add(iMeasures);

        for (iBeats = 0; iBeats < beatsPerMeasure; iBeats += 1) {
          $curBeat = $("<div>", { class: opts.beatClass });
          // $curBeat[iMeasures].classList.add(iBeats);
    ...
    function buildTrack() {
      ...
      $track.addClass(opts.trackClass).append(
        ...
        $pattern.addClass(opts.patternClass).addClass(trackId.toString())
      );
    ...
    document.addEventListener("keydown", function (e) {
      ...
      //       on = $(this).toggleClass(opts.onClass).hasClass(opts.onClass);
      //       $(this).toggleClass(getStepIndex(this).toString());
      //       events.trigger(
    ...
    $(document).on("addStep", function () {
      $(".step").each(function (index) {
        if ($(this).hasClass("on")) {
          // var on = $(this).toggleClass(opts.onClass).hasClass(opts.onClass);
          on = $(this).toggleClass(getStepIndex(this).toString());
          events.trigger(
            "ui.step.toggled", // Event name
            [trackId, getStepIndex(this), on], // Params
            this // Context
