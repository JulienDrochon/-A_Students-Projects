...
(function (App, $) {
  ...
  App.ui.TrackManager = function (gridSelector, controlsSelector, options) {
    ...
    function getTrackById(trackId) {
      if (typeof trackIndex[trackId] === "undefined") {
        return false;
      }
    ...
    function stepTick(stepIndex) {
      var $tracks = $grid.find("." + opts.trackClass),
        $allSteps = $tracks.find("." + opts.stepClass),
        $steps = $tracks.find(
          "." + opts.stepClass + ":eq(" + stepIndex.toString() + ")"
        );
    ...
    function stepsChanged() {
      ...
      $patterns.width(
        measures *
          $patterns.find("." + opts.measureClass + ":first-child").outerWidth()
      );
    ...
    $clearButton = $("<button>", { class: "icon-trash" })
      ...
      .click(function () {
        ...
        App.ui.dialogs
          .OkCancel({
            content:
              "Are you sure you want to clear the pattern? You can not undo this.",
            okButtonText: "Yes",
            cancelButtonText: "No",
