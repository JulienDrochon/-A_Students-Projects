...
(function (App, $) {
  ...
  dialogs.DialogError = function (message) {
    ...
    this.message =
      message || "An error occured. No more information is available.";
  };

  dialogs.Dialog = (function () {
    var dialogCounter = 0, // 'private static'
      ...
      NewDialog = function (options) {
        ...
        this.getOverlayZIndex = function () {
  ...
  dialogs.Dialog.cleanOptions = function (options, ignoredOptions) {
    ...
    ignoredOptions =
      ignoredOptions instanceof Array
        ? ignoredOptions
  ...
  dialogs.Dialog.prototype.spawn = function () {
    var htmlId = "dc_dialog_" + this.getDialogId(),
      i = 0,
      iMax = 0,
    ...
    if (dialogs.draggableAvailable) {
      ...
      this.$box.css(
        "left",
        ($(document).outerWidth() - this.$box.outerWidth()) / 2 + "px"
      );
    ...
    this.$box.css(
      "top",
      ($(document).outerHeight() - this.$box.innerHeight()) / 2 + "px"
    );
  ...
  dialogs.Dialog.prototype.close = function () {
    ...
    if ($element === null) {
      throw new dialogs.DialogError(
        "Cannot close dialog before it has been spawned"
      );
    }

    if (
      typeof $element.remove !== "function" ||
      typeof $element.fadeOut !== "function"
    ) {
      $element = $($element);
    }
  ...
  dialogs.Alert = function (options) {
    var cleanOptions = dialogs.Dialog.cleanOptions,
      i,
      iMax,
      defaultOptions = {
        buttonText: "Ok",
    ...
    if (!this instanceof dialogs.Alert) {
      //ensure instantiating
    ...
    options.buttons = [
      {
        label:
          typeof options.buttonText === "string" ? options.buttonText : "Ok",
        action: "closeDialog",
      },
    ];
  ...
  dialogs.OkCancel = function (options) {
    ...
    if (!this instanceof dialogs.OkCancel) {
      //ensure instantiating
  ...
  dialogs.MultipleChoice = function (options) {
    ...
    if (!this instanceof dialogs.MultipleChoice) {
      //ensure instantiating
    ...
    if (options.choiceType === "select") {
      $choicesSelect = $("<select>", { name: "dialog-choice" })
        .append($("<option>", { value: -1 }).html(options.selectPlaceholder))
      ...
      switch (options.choiceType) {
        ...
        case "select":
          $choicesSelect.append(
            $("<option>", { value: options.choices[i].value }).html(
              options.choices[i].label
            )
          );
          break;
  ...
  dialogs.Prompt = function (options) {
    ...
    if (!this instanceof dialogs.Prompt) {
      //ensure instantiating
    ...
    // $inputField.bind(function (event) {
    //   self.returnValue = $inputField.val();
    //   self.close();
    // });
